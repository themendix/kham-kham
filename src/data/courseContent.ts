...
export function getSubjectContent(categoryId: string): Promise<Course[]> {
  ...
  if (!loader) return Promise.resolve(MISC_COURSES.filter((c) => c.categoryId === categoryId));
...
/**
 * Précharge en tâche de fond, sans bloquer le rendu, tout le contenu du catalogue (Histoire +
 * Géographie + matières émergentes). Idempotent et mémorisé : un seul chargement réseau même
...
export function preloadAllSubjectContent(): Promise<Course[]> {
  if (!allContentPromise) {
    allContentPromise = Promise.all(Object.keys(SUBJECT_LOADERS).map(getSubjectContent)).then(
      (groups) => [...groups.flat(), ...MISC_COURSES],
    );
  }
